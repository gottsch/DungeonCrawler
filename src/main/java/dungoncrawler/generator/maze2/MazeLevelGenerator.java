...
public class MazeLevelGenerator {
    ...
    private static final int DEFAULT_ATTEMPTS_MAX = 200;
    ...
    private int attemptsMax = DEFAULT_ATTEMPTS_MAX;
     ...
     * @param builder
     */
    public MazeLevelGenerator(Builder builder) {
        ...
        this.attemptsMax = builder.attemptsMax;
    ...
    public Optional<ILevel2D> generate() {
        ...
        List<IRoom2D> rooms = generateRooms(this.meanFactor);

        // NOTE This is moot as rooms are filtered as they are added
        // filter some rooms out
//        rooms = removeRoomsBelowAreaMean(rooms, meanFactor);
        LOGGER.debug("remaining rooms after filter -> {}", rooms.size());
        // map rooms to regions
        Map<Integer, MazeRegion2D> regions = mapRegions(rooms);

        // populate the level properties
        level.setRegionMap(regions);
        level.setStartRoom(startRoom);
        level.setEndRoom(endRoom);
        // add the rooms to the level
        level.setRooms(rooms);
        // load rooms into the buildMap
        ...
        // TODO handle discovery failure... either roll up, or attempt to re-carve and re-discover
    ...
    private void validate() {
        // validate start room and end room properties if they are provided
        if (startRoom != null) {
            if (startRoom.getHeight() % 2 != 1) {
                startRoom.setHeight(startRoom.getHeight()-1);
            }
            if (startRoom.getWidth() % 2 != 1) {
     ...
     * @return
     */
    public List<IRoom2D> generateRooms(double localMeanFactor) {
        ...
        if (startRoom == null) {
            while (startRoom == null) {
                Optional<IRoom2D> room = generateRoom(xRange, yRange, minSize, maxSize, levelBoundary, 2, maxDegrees, random);
                startRoom = room.orElse(null);
            }
            startRoom.setStart(true);
        }
        // assign an id to the room (whether provided or generated)
        startRoom.setId(idGenerator.next());
        rooms.add(startRoom);

        // generate an end room
        boolean hasIntersections = false;
        if (endRoom == null) {
            while (endRoom == null || hasIntersections) {
                Optional<IRoom2D> room = generateRoom(xRange, yRange, minSize, maxSize, levelBoundary, 1, 1, random);
                endRoom = room.orElse(null);
                // ensure endRoom doesn't overlap startRoom

                if (endRoom != null) {
                    hasIntersections = hasIntersections(room.get().getBox(), rooms);
                }
            }
            endRoom.setEnd(true);
        }
        endRoom.setId((idGenerator.next()));
        rooms.add(endRoom);

        double failureRun = 0;
        // random rooms
        for (int attemptCount = 0; attemptCount < attemptsMax; attemptCount++) {
            if (failureRun > 0 && failureRun / attemptsMax > 0.1) {
            ...
            // TODO this would be more efficient if using a Interval-BST
    ...
    public Optional<IRoom2D> generateRoom(int xRange, int yRange, int minSize, int maxSize,
        ...
        // TODO the x and y cannot be more than nSize in difference
        // ex. maxDiff = 5
        // min=5, max=19; NO
        // min=5 max=10; YES
    ...
    public boolean discoverConnectors(ILevel2D level) {
        ...
        boolean customConnectorsSuccess = false;
        for(IRoom2D room : level.getRooms()) {
            if (!room.getDoorways().isEmpty()) {
                ...
                for(Coords2D door : room.getDoorways()) {
                    int x = door.getX();
                    int y = door.getY();
                    customConnectorsSuccess |= generateConnector(level, x, y);
                }

                // TODO what if none of the doors were valid?!!
        ...
        for (int x = 1; x < level.getWidth()-1; x++) {
            for (int y = 1; y < level.getHeight()-1; y++) {
                ...
                if (level.getGrid().getId(x, y) == WALL) {
                    generateConnector(level, x, y, ignoreIds);
                }
            }
        }
        return true;
    ...
    private boolean generateConnector(ILevel2D level, int x, int y) {
        return generateConnector(level, x, y, null);
    }
     ...
     * @param y
     */
    private boolean generateConnector(ILevel2D level, int x, int y, List<Integer> ignoreIds) {
        ...
        if (northId >= idGenerator.getStart() && southId >= idGenerator.getStart() && northId != southId) {
            // get regions
            MazeRegion2D region1 = level.getRegionMap().get((int)northId);
            MazeRegion2D region2 = level.getRegionMap().get((int)southId);
            ...
            level.getConnectors().add(connector);
            ...
            MazeRegion2D region1 = level.getRegionMap().get((int)eastId);
            MazeRegion2D region2 = level.getRegionMap().get((int)westId);
    ...
    public void mergeRegions(ILevel2D level, Random random) {
        ...
        while (!level.getConnectors().isEmpty()) {
            connectors.clear();
            // randomly select a connector
            Connector2D connector = level.getConnectors().get(random.nextInt(level.getConnectors().size()));

            /*
             * gather all connectors that match the regions
             */
            // scan list for start room connectors
            level.getConnectors().forEach(c -> {
            ...
            connectors.forEach(c -> {
                ...
                // TODO test to remove (using config property of room)
                if (random.nextDouble() > 0.955 && !ignoreList.contains(c) && connectorCount.get() < degrees) {
                    // TODO ensure this only goes to the max determined by the config proper
                    // ex if (c.getRegion1.isRoom()) { get room.degrees }; count < degree
                    // TODO somehow would have to keep track of the count per room/region
                    // build extra door
     ...
     * @param startCoords
     */
    public void prims(ILevel2D level, Coords2D startCoords) {
        List<Tile> activeList = new ArrayList<>();
        Random random = new Random();
        Map<Direction2D, Tile> neighbors = new HashMap<>();
        Map<Direction2D, Tile> passages = new HashMap<>();
        int maxRun = random.nextInt(250) + 250; // max run of 250-500 // TODO add a config setting
        // create tile
        Tile tile = new Tile(startCoords, Direction2D.SOUTH);
...
//        region.addTile(tile);
        ...
        level.getRegionMap().put(region.getId(), region);
        ...
        while(!activeList.isEmpty()) {
            ...
            if (level.getGrid().getId(active.getX(), active.getY()-1) == ROCK) {
                if (level.getGrid().getId(active.getX(), active.getY()-2) == ROCK) {
                    neighbors.put(Direction2D.NORTH, new Tile(active.getX(), active.getY()-2, Direction2D.NORTH));
                    passages.put(Direction2D.NORTH, new Tile(active.getX(), active.getY()-1, Direction2D.NORTH));
                } else {
            ...
            if (level.getGrid().getId(active.getX(), active.getY()+1) == ROCK) {
                if (level.getGrid().getId(active.getX(), active.getY()+2) == ROCK) {
                    neighbors.put(Direction2D.SOUTH, new Tile(active.getX(), active.getY() + 2, Direction2D.SOUTH));
                    passages.put(Direction2D.SOUTH, new Tile(active.getX(), active.getY() + 1, Direction2D.SOUTH));
                } else {
                    level.getGrid().setId(active.getX(), active.getY()+1, WALL);
                }
            }
            if (level.getGrid().getId(active.getX()+1, active.getY()) == ROCK) {
                if (level.getGrid().getId(active.getX()+2, active.getY()) == ROCK) {
                    neighbors.put(Direction2D.EAST, new Tile(active.getX()+2, active.getY() , Direction2D.EAST));
                    passages.put(Direction2D.EAST, new Tile(active.getX()+1, active.getY(), Direction2D.EAST));
                } else {
            ...
            if (level.getGrid().getId(active.getX()-1, active.getY()) == ROCK) {
                if (level.getGrid().getId(active.getX()-2, active.getY()) == ROCK) {
                    neighbors.put(Direction2D.WEST, new Tile(active.getX() - 2, active.getY(), Direction2D.WEST));
                    passages.put(Direction2D.WEST, new Tile(active.getX() - 1, active.getY(), Direction2D.WEST));
                } else {
                    level.getGrid().setId(active.getX()-1, active.getY(), WALL);
                }
            }
...
//            region.addTile(passage);
    ...
    public static class Builder {
        ...
        public int attemptsMax = DEFAULT_ATTEMPTS_MAX;
        ...
        public MazeLevelGenerator build() {
            return new MazeLevelGenerator(this);
        }
    }
}
